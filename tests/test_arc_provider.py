...
class ArcProviderTests(unittest.TestCase):
    ...
    def test_creates_and_caches_wallet(self, mock_get_client):
        ...
        with patch(
            ...
            # Checks Circle's ref_id index first (in case the local cache
            # was wiped by a redeploy) before creating a new wallet.
            get_wallets.assert_called_once()
            create_wallet.assert_called_once()
            # ref_id is set right after creation so this wallet is findable
            # via get_wallets even if the local cache disappears later.
            update_wallet.assert_called_once()
    ...
    @patch("rpc.arc_provider._get_client")
    def test_finds_existing_wallet_via_ref_id_after_cache_loss(self, mock_get_client):
        # Simulates the real Render symptom: local data/arc_state.json was
        # wiped by a redeploy, but the wallet still exists on Circle's side
        # from before. get_or_create_wallet must find and reuse it, not
        # silently hand the same user a brand-new, empty wallet.
        mock_get_client.return_value = Mock()

        existing_wallet = Mock(id="wallet-old", address="0xOldRealDepositAddress")
        lookup_response = Mock()
        lookup_response.data.wallets = [existing_wallet]
        ...
        with patch(
            ...
            result = arc_provider.get_or_create_wallet("0xReturningUser")

        self.assertFalse(result["no_data"])
        self.assertEqual(result["wallet_id"], "wallet-old")
        self.assertEqual(result["deposit_address"], "0xOldRealDepositAddress")
        # Must NOT create a second wallet when one is already found on Circle.
    ...
    @patch("rpc.arc_provider._get_client")
    def test_ref_id_set_matches_the_user_key(self, mock_get_client):
        mock_get_client.return_value = Mock()

        wallet = Mock(id="wallet-2", address="0xDep2")
        create_response = Mock()
        create_response.data.wallets = [wallet]
        empty_lookup = Mock()
        empty_lookup.data.wallets = []
        ...
        with patch(
            ...
            arc_provider.get_or_create_wallet("0xUserWithMixedCase".upper())

        update_wallet.assert_called_once()
        _, kwargs = update_wallet.call_args
